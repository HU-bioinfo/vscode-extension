...
```shell
> work-env@1.0.6 unit-test
> cross-env NODE_ENV=test VSCODE_MOCK=1 mocha --require ts-node/register --ui bdd --extension ts,tsx --recursive 'test/**/*.test.ts' --exclude 'test/runTest.ts' --timeout 60000
...
```shell
> work-env@1.0.6 integration-test
> npm run compile && cross-env VSCODE_MOCK=0 node ./out/test/runTest.js

テストモード: 統合テスト
テストヘルパー: 実際のVS Code APIを使用
...
-------------------|---------|----------|---------|---------|----------------------------------------
All files          |    70.2 |     56.8 |    65.3 |    71.5 |
 error-handlers.ts |    80.2 |     78.3 |    76.0 |    80.2 | ...
 extension.ts      |    82.1 |     62.5 |    94.5 |    82.6 | ...
 test-helper.ts    |    52.3 |     32.8 |    42.1 |    54.8 | ...
-------------------|---------|----------|---------|---------|----------------------------------------
```
...
1. **無限再帰問題**:
   - 原因: モック環境と統合テスト環境での関数呼び出しの競合
   - 解決策: テストモードに基づく条件分岐ロジックを実装

2. **型の問題**:
   - 原因: VS Code API の命名の衝突
   - 解決策: インポート時に `import * as vscodeTypes from 'vscode'` として区別

3. **テスト失敗の問題**:
   - 原因: 実際の VS Code 環境でモックリセット関数が呼ばれる
   - 解決策: 環境に基づいてスキップロジックを追加
...
| error-handlers.ts   | 87.3%          | 89.83%   | 80%    | 87.3%  |
| extension.ts        | 67.6%          | 50%      | 85.29% | 68.23% |
| test-helper.ts      | 58.17%         | 20.89%   | 50%    | 59.5%  |

※ブランチカバレッジが閾値（50%）を下回っていますが、主に Docker Installer モジュールのカバレッジが低いことが原因です。

### リソース管理の変更

2024-04-05 の変更で、リソース管理方法を改善しました：

1. **テンプレートファイルの移動**

   - `.devcontainer`ディレクトリと設定ファイルを`resources/templates/devcontainer_template`に移動
   - テンプレートファイルに`.template`拡張子を追加して識別しやすく

2. **リソースアクセス方法の改善**

   - `vscode.Uri.joinPath`を使用して安全なリソースアクセスを実装
   - エクステンションのビルド時と実行時の両方で動作するよう改善
...
1. **Docker Installer のテストカバレッジ向上**

   - 現在は Docker Installer モジュールのカバレッジが特に低い（13.28%）
   - 各 OS とディストリビューションに対するモックテストの追加が必要
