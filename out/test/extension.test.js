...
describe('Extension Test Suite', () => {
    beforeEach(() => {
        ...
        test_helper_1.vscode.env.openExternal.returns(Promise.resolve(true));
    });
    afterEach(() => {
        sinon.restore();
    });
    ...
    it('Docker Compose設定情報の収集', async () => {
        ...
        test_helper_1.expectation.errorMessageShown('プロジェクトフォルダが選択されていません');
        // キャッシュフォルダが選択されなかった場合
        (0, test_helper_1.resetMocks)();
        test_helper_1.vscode.window.showOpenDialog.onFirstCall().resolves([{ fsPath: '/test/project' }]);
        test_helper_1.vscode.window.showOpenDialog.onSecondCall().resolves(undefined);
        assert.strictEqual(await extension.collectDockerComposeConfig(), null);
        test_helper_1.expectation.errorMessageShown('キャッシュフォルダが選択されていません');
        // GitHub PATが入力されなかった場合
        (0, test_helper_1.resetMocks)();
        test_helper_1.vscode.window.showOpenDialog.onFirstCall().resolves([{ fsPath: '/test/project' }]);
        test_helper_1.vscode.window.showOpenDialog.onSecondCall().resolves([{ fsPath: '/test/cache' }]);
        test_helper_1.vscode.window.showInputBox.resolves(undefined);
        assert.strictEqual(await extension.collectDockerComposeConfig(), null);
        test_helper_1.expectation.errorMessageShown('GitHub PATが必要です');
    });
    it('テンプレートファイル処理', () => {
        // 成功の場合
        test_helper_1.fsMock.readFileSync.returns('value: {{GITHUB_PAT}}, path: {{CACHE_FOLDER}}, project: {{PROJECT_FOLDER}}');
        assert.strictEqual(extension.processTemplateFile('/template.yml', '/output.yml', {
            githubPat: 'test-pat',
            cacheFolder: '/test/cache',
            projectFolder: '/test/project'
        }), true);
        test_helper_1.expectation.fileCreated('/output.yml', 'value: test-pat, path: /test/cache, project: /test/project');
        // 失敗の場合
        test_helper_1.fsMock.readFileSync.throws(new Error('File not found'));
        assert.strictEqual(extension.processTemplateFile('/template.yml', '/output.yml', {
            githubPat: 'test-pat',
            cacheFolder: '/test/cache',
            projectFolder: '/test/project'
        }), false);
        test_helper_1.expectation.errorMessageShown('docker-compose.ymlの生成中にエラーが発生しました');
    });
    it('フォルダーのコピー', () => {
        // フォルダーとファイルパス
        const source = '/source';
    ...
    it('setupDevContainerのテスト', () => {
        ...
        test_helper_1.fsMock.readdirSync.returns(['file1.txt']);
        test_helper_1.fsMock.lstatSync.returns({ isDirectory: () => false });
        extension.setupDevContainer(context, targetPath);
        assert.ok(test_helper_1.fsMock.existsSync.called);
        assert.ok(test_helper_1.fsMock.copyFileSync.called);
    });
    ...
    it('activate & deactivate関数での追加カバレッジ', () => {
        ...
        extension.deactivate();
        // アサーション
        assert.strictEqual(test_helper_1.vscode.commands.registerCommand.callCount, 2);
    });
    ...
    it('processTemplateFile関数のエラー処理テスト', () => {
        // ファイル読み込みエラーの場合
        test_helper_1.fsMock.readFileSync.throws(new Error('ファイル読み込みエラー'));
        const result = extension.processTemplateFile('/template.yml', '/output.yml', {
            githubPat: 'test-pat',
            cacheFolder: '/test/cache',
            projectFolder: '/test/project'
        });
        assert.strictEqual(result, false);
        assert.ok(test_helper_1.vscode.window.showErrorMessage.calledWith(sinon.match('docker-compose.ymlの生成中にエラーが発生しました。')));
        // エラーハンドラーが呼ばれたことを確認
